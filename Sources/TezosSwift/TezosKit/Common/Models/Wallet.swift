...
//  }
  ...
  public init?(mnemonic: String, passphrase: String = "", signingCurve: EllipticalCurve = .ed25519) {
      do {
          ...
          self.init(address:keypair.address , publicKey: PublicKey(bytes: keypair.publicKey.bytes,signingCurve:.ed25519), secretKey: SecretKey(keypair.secretKey.bytes), mnemonic: mnemonic)
      }catch {
          return nil
      }
  }
